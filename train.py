...
db_config = {
    ...
    "user": "root",
    "password": "",
    "database": "intents"  # Replace with your database name
}

connection = mysql.connector.connect(**db_config)
cursor = connection.cursor()

# Fetch intent data from the database
...
for tag, pattern, response in intents_data:
    tags.append(tag)
    w = tokenize(pattern)
    all_words.extend(w)
    xy.append((w, tag))
...
print(f'Final loss: {loss.item():.4f}, Accuracy: {acc:.2f}%')
